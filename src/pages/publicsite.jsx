...
export default function PublicSite() {
  ...
  useEffect(() => {
    document.title = data?.business?.name
      ? `${data.business.name} — MiSitioFácil`
      : 'MiSitioFácil'
  }, [data])
  ...
  // horario (acepta operatingHours u hours)
  const hours = business?.operatingHours || business?.hours || null
  ...
  return (
    <div className="row g-3">
        ...
        <div className="card p-0 overflow-hidden">
          ...
          <div className="p-4">
            <div className="d-flex align-items-center gap-3">
              <div
                style={{
                  width: 64, height: 64, borderRadius: 16, display: 'grid',
                  placeItems: 'center', background: 'rgba(255,255,255,.06)',
                  border: '1px solid #2a3346', fontWeight: 700, fontSize: 22
                }}
              >
                {(business?.name || '?')
                  .split(' ')
            ...
            <div className="d-flex flex-wrap gap-2">
              ...
              {business?.instagram && (
                <a className="btn btn-soft btn-sm" target="_blank" rel="noopener noreferrer" href={business.instagram}>
                  <i className="bi bi-instagram me-1" /> Instagram
                </a>
              )}
              {business?.facebook && (
                <a className="btn btn-soft btn-sm" target="_blank" rel="noopener noreferrer" href={business.facebook}>
      ...
      {hours && (
        <div className="col-12">
          <div className="card p-3">
            <h6 className="mb-1"><i className="bi bi-clock me-1" /> Horario de atención</h6>
            <ul className="mb-0 ps-3">
              {Object.entries(hours).map(([day, val]) => (
                <li key={day}>
                  <strong>{LABEL[day] || day}:</strong>{' '}
                  {val?.isOpen ? `${val.openTime} - ${val.closeTime}` : 'Cerrado'}
                </li>
              ))}
            </ul>
          </div>
